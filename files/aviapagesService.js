...
// ─── PRICE EXTRACTOR ─────────────────────────────────────────────
// Aviapages paid response mein price in fields se aata hai.
// Hum saare possible keys check karte hain — jo bhi non-zero mile use karo.
function extractRealPrice(response) {
  if (!response) return null;

  // Direct top-level price fields (most likely for paid accounts)
  const directFields = [
    'price',
    'total_price',
    'charter_price',
  ...
  // Nested: response.result.price / response.data.price
  ...
  // Legs array mein price ho sakta hai
  const legs = response.flights || response.legs || [];
  if (legs.length > 0) {
    const legPrices = legs
      .map((l) => l.price || l.cost || l.amount || 0)
      .filter((p) => p > 0);
  ...
  return null; // koi price nahi mila
}
...
//  Available charter aircraft list fetch karo
// ─────────────────────────────────────────────────────────────────
export async function fetchCharterAircraft(apiKey, { departure, arrival, category, passengers }) {
  ...
  // Response: { count, next, previous, per_page, results: [ aircraft... ] }
}

// ─────────────────────────────────────────────────────────────────
//  STEP 0 — POST /v3/charter_prices/
//  Correct body format (as per Aviapages docs):
//  {
//    legs: [{ departure_airport: { icao }, arrival_airport: { icao },
//             pax, departure_datetime }],
//    aircraft: [{ ac_class: "Midsize" }],
//    currency_code: "USD"
//  }
//  Response: { price: 20000, price_min: null, price_max: null, currency_code: "EUR" }
// ─────────────────────────────────────────────────────────────────

// Airport code ko proper object mein wrap karo
// IATA = 3 chars → { iata: "BOM" },  ICAO = 4 chars → { icao: "VABB" }
function airportObj(code) {
  const c = (code || '').toUpperCase().trim();
  return c.length === 4 ? { icao: c } : { iata: c };
}

// charter_aircraft API class name → charter_prices ac_class format
// e.g. "Light Jet" → "Light", "Super Midsize" → "Super midsize"
    ...
    legs: [
      {
        ...
        departure_datetime:  date ? `${date}T00:00` : `${new Date().toISOString().slice(0, 10)}T00:00`,
      },
    ],
    currency_code: 'USD',
  };

  // Aircraft class filter — normalize to API format
  const normalizedClass = normalizeAcClass(aircraftClass);
  if (normalizedClass) {
    body.aircraft = [{ ac_class: normalizedClass }];
  }
...
//  Flight time + route calculate karo
// ─────────────────────────────────────────────────────────────────
export async function fetchFlightCalculator(apiKey, { departure, arrival, aircraft, date }) {
  const body = {
  ...
  // Response: { time: { airway: 536 }, airport: {...}, aircraft: "..." }
}

// ─────────────────────────────────────────────────────────────────
//  API 3 — POST /v3/price_calculator/
//  REAL price fetch karo (paid account required)
//
//  Aapke paid account mein yeh extra fields aate hain:
//   - profile / pricing_profile  → aapka cost profile ID
//   - currency                   → response currency
//   - pax                        → passenger count
// ─────────────────────────────────────────────────────────────────
export async function fetchPriceCalculator(
  apiKey,
  { departure, arrival, aircraft, date, passengers, profile }
) {
  const body = {
    departure_airport: departure.toUpperCase(),
    arrival_airport:   arrival.toUpperCase(),
  ...
  if (passengers) body.pax              = parseInt(passengers, 10); // "pax" field bhi try karo
  if (passengers) body.passengers       = parseInt(passengers, 10);
  if (profile)    body.profile          = profile;   // pricing profile ID (dashboard se lo)
  if (profile)    body.pricing_profile  = profile;

  return apiCall(apiKey, 'POST', '/v3/price_calculator/', body);
  // Paid response mein expected: { price/total_price, currency, flights: [...] }
}

// ─────────────────────────────────────────────────────────────────
//  MASTER FUNCTION
//  Teeno APIs ko chain karo + real price extract karo
// ─────────────────────────────────────────────────────────────────
export async function fetchCompleteFlightData(apiKey, searchParams) {
  ...
  // ── STEP 1: Aircraft list ──────────────────────────────────────
  console.log('\n════ STEP 1: Charter Aircraft ════');
  const aircraftData = await fetchCharterAircraft(apiKey, {
    departure, arrival, category, passengers,
  });

  const aircraftList = aircraftData.results || [];
  if (aircraftList.length === 0) {
    return { enrichedList: [], total: 0 };
  }

  // ── STEP 2 + 3 + PRICE: Har aircraft ke liye parallel ─────────
  const enrichedList = await Promise.allSettled(
    aircraftList.slice(0, 15).map(async (ac) => {
      ...
      let flightRawResponse = null;
      try {
        const fc = await fetchFlightCalculator(apiKey, {
          departure, arrival, aircraft: aircraftName, date,
        });
        flightRawResponse    = fc;
      ...
      try {
        const pc = await fetchPriceCalculator(apiKey, {
          departure, arrival, aircraft: aircraftName,
          date, passengers, profile,
        });
        priceRawResponse = pc;

        // ✅ Real price extract karo
        priceResult = extractRealPrice(pc);

        // Legs (flight_time per leg)
        legs = pc?.flights || pc?.legs || [];
        if (legs.length > 0) {
          legTotalMinutes = legs.reduce((s, l) => s + (l.flight_time || 0), 0);
        }

        if (priceResult) {
          console.log(`[Price] ✅ Real price found: ${priceResult.amount} ${priceResult.currency} (via "${priceResult.source}")`);
        } else {
          console.warn(`[Price] ⚠️ No price field found in response for ${aircraftName}`);
          console.warn('[Price] All keys in response:', Object.keys(pc || {}));
        }
      } catch (err) {
        console.warn(`[PriceCalc] ${aircraftName}:`, err.message);
      }

      // Best flight time: API2 preferred, API3 sum as fallback
      const bestFlightTime = flightTimeMinutes || legTotalMinutes;

      // ── STEP 0: Charter Prices — per aircraft class ────────────
      console.log(`\n════ STEP 0: Charter Price — ${aircraftClass || 'Any'} ════`);
      let classPrice = null;
      try {
        const cp = await fetchCharterPrices(apiKey, {
          departure, arrival, passengers, date,
      ...
      // ── Merge: price_calculator (exact) wins; charter_prices as fallback ──
      ...
      // For range display — show min/max only if both exist
      ...
      // isRangePrice = true → show min–max; false → show single price
      const isRangePrice = !hasExactPrice && !!(classPrice?.min && classPrice?.max);

      const realPriceMidFormatted = realPriceMid
        ? formatCurrency(realPriceMid, realPriceCurrency)
        : null;

      const realPriceINR = realPriceMid && realPriceCurrency === 'USD'
        ? formatINR(realPriceMid)
        : null;
      ...
      return {
        ...
        // ── API 3 (price_calculator — exact, paid only) ──
        legs,
        legTotalMinutes,
        priceRawResponse,

        // ── PRICE (used by ResultsScreen) ─────────────────────────
        hasRealPrice,
        isRangePrice,           // true → show min–max range; false → single price
        realPriceMin,           // charter_prices price_from
        realPriceMax,           // charter_prices price_to
        realPriceMid,           // midpoint or exact price
        realPriceMidFormatted,  // formatted midpoint / exact
        realPriceCurrency,
        realPriceINR,
        realPriceAmount:     realPriceMid,   // for sorting
